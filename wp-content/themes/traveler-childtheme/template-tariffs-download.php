...
Template Name: Tariffs Download
...
$data = decryptData($_GET["file"]);

$arrVirtualFilePath = explode("/",$data);
$countVirtualFIlePath = 0;
$strPhysicalFilePath = ''; 

 foreach ($arrVirtualFilePath as $arrFilePath) { 
	if ($countVirtualFIlePath > 3){
		
		$strPhysicalFilePath .= $arrFilePath;
		
		if ($countVirtualFIlePath < (sizeof($arrVirtualFilePath) - 1)){
			$strPhysicalFilePath .= '\\';
		}
		 
    }
    $countVirtualFIlePath += 1;
 }
 
$strPhysicalFilePath = '/home2/travster/public_html/experiencesinasia/wp-content/' . str_replace("\\","/", $strPhysicalFilePath);
//echo $strPhysicalFilePath . '<br/>';
//$strPhysicalFilePath = str_replace("wp-content\\","", __DIR__) . $strPhysicalFilePath;


//echo $strPhysicalFilePath;
//exit();
...
$mime_types['pptx']   ='application/vnd.ms-powerpoint';
